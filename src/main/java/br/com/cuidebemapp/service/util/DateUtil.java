...
public class DateUtil {
	...
	public static List<java.time.OffsetDateTime> listDates(java.time.OffsetDateTime inicio,
			java.time.OffsetDateTime fim, String hour) {
		List<java.time.OffsetDateTime> dates = new ArrayList<java.time.OffsetDateTime>();
		java.time.OffsetDateTime inicioInstant = convertHour(inicio, hour);
		java.time.OffsetDateTime fimInstant = convertHour(fim, hour);

		while (inicioInstant.compareTo(fimInstant) <= 0) {
			if (inicioInstant.isAfter(java.time.OffsetDateTime.now())) {
				dates.add(inicioInstant);
			}
	...
	public static List<java.time.OffsetDateTime> listDates(java.time.OffsetDateTime inicio,
			java.time.OffsetDateTime fim, String hour, int repeatHour) {
		List<java.time.OffsetDateTime> dates = new ArrayList<java.time.OffsetDateTime>();
		java.time.OffsetDateTime inicioInstant = convertHour(inicio, hour);
		java.time.OffsetDateTime fimInstant = convertHour(fim, hour);
		while (inicioInstant.compareTo(fimInstant) <= 0) {
	...
	public static List<Date> listDates(Date inicio, Date fim, String hour, Integer[] daysOfWeek) {
		...
		Instant fimInstant = convertHour(fim, hour).toInstant();
		while (inicioInstant.compareTo(fimInstant) <= 0) {
			if (days.contains(LocalDateTime.ofInstant(inicioInstant, ZoneId.of("America/Sao_Paulo"))
	...
	public static List<java.time.OffsetDateTime> listDates(java.time.OffsetDateTime inicio,
			java.time.OffsetDateTime fim, String hour, Integer[] daysOfWeek) {
		List<Integer> days = Arrays.asList(daysOfWeek);
		List<java.time.OffsetDateTime> dates = new ArrayList<java.time.OffsetDateTime>();
		...
		while (inicioInstant.compareTo(fimInstant) <= 0) {
			if (days.contains(inicioInstant.getDayOfWeek().getValue())) {
				dates.add(inicioInstant);
			}
			inicioInstant = inicioInstant.plus(1, ChronoUnit.DAYS);
		}
	...
	public static java.time.OffsetDateTime convertHour(java.time.OffsetDateTime date, String hour) {
		java.time.OffsetDateTime dd = java.time.OffsetDateTime.ofInstant(date.toInstant(), ZoneId.of("America/Sao_Paulo"));
		String[] values = hour.split(":");
		int _hour = Integer.valueOf(values[0]);
		int minute = Integer.valueOf(values[1]);
		dd = dd.withHour(_hour);
		dd = dd.withMinute(minute);
		return dd;
	}

	public static String dayOfWeek(Date date) {
		return LocalDateTime.ofInstant(date.toInstant(), ZoneId.of("America/Sao_Paulo")).getDayOfWeek()
	...
	public static String getDifHourMinute(Date initial, Date end) {
		...
		return String.format("%02d:%02d", TimeUnit.MILLISECONDS.toHours(millis), TimeUnit.MILLISECONDS.toMinutes(millis)
